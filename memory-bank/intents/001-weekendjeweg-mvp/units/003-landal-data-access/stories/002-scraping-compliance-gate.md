...
Status: blocked-before-real-run
...
- Compliance review records include target domain, robots check, terms check, rate limit policy, approval state, approval timestamp, and notes.
...
Scraping may be implemented as fallback code, but it must not run against real Landal pages before approval.
